...
Braki w kolumnie Loan_Amount_Term sytuacja może wyglądać podobnie - mogą to być klienci, którzy nie wypełnili wartości w kolumnie, zakładając domyślne ustawienia banku. Dlatego braki w kolumnach LoanAmount i Loan_Amount_Term również zostają uznane za potencjalne MNAR.

Braki w kolumnie Gender mogą reprezentować osoby, które nie chciały podawać tej informacji, uznając, że płeć nie powinna mieć wpływu na decyzje kredytową. Braki w kolumnie Dependents mogą oznaczać świadome pominięcie pytania w obawie przed negatywnym wpływem odpowiedzi na zdolność kredytową lub po prostu z braku obowiązku. Dlatego braki tej zmiennej, braki zmiennej Married oraz braki zmiennej Gender zostają uznane za MNAR.
...
Obserwacje z brakiem danych w kolumnie LoanAmount uzupełnione będą za pomocą techniki Hot Deck, w której brakująca wartość zastępowana wartością od klienta o najbardziej podobnych cechach. 
